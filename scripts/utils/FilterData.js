class FilterData {
  ...
  filterData() {
    this.searchBar.addEventListener("input", (e) => {
      ...
      const res1 = filteredArrayDescription.filter(
        (el1) => !filteredArrayName.find((el2) => el1.id === el2.id)
      );
      for (const result1 in res1) {
        filteredArrayName.push(res1[result1]);
      }
      const res2 = filteredArrayIngredient.filter(
        (el1) => !filteredArrayName.find((el2) => el1.id === el2.id)
      );
      ...
      if (value.length >= 3) {
        ...
        this.filterIngredientsOnClick(filteredResult);
      } else {
        this.recipeFactory.createRecipes(this.recipes);
        this.itemFactory.createItems(this.recipes);
      }
  ...
  filterIngredientsOnClick(recipes) {
    ...
    document
      .querySelectorAll(".search-section .filters .ingredients-list li")
      .forEach((li) =>
        li.addEventListener("click", (e) => {
          let selectedTag = e.target.dataset.set;
          const target = e.target;
          const data = recipes.filter((recipe) =>
            recipe.ingredients.some((ingredient) => {
              return ingredient.ingredient.toLowerCase().includes(selectedTag);
            })
          );
          this.itemFactory.createItems(data);
          this.recipeFactory.createRecipes(data);
          target.style.display = "none";
          return this.tagArray.push(selectedTag);
        })
      );
  }
  ...
  filterApplianceOnClick(recipes) {
    ...
    document
      .querySelectorAll(".search-section .filters .appliances-list li")
      .forEach((li) =>
        li.addEventListener("click", (e) => {
          let selectedTag = e.target.dataset.set;
          const data = recipes.filter((recipe) =>
  ...
  filterUstensilOnClick(recipes) {
    document
      .querySelectorAll(".search-section .filters .ustensils-list li")
      .forEach((li) =>
        li.addEventListener("click", (e) => {
          let selectedTag = e.target.dataset.set;
          const data = recipes.filter((recipe) =>
            recipe.ustensils.some((ustensil) => {
              return ustensil.toLowerCase().includes(selectedTag);
            })
          );
          this.itemFactory.createItems(data);
          this.recipeFactory.createRecipes(data);
